...
public class LibraryManagementSystemImpl implements LibraryManagementSystem {
    ...
    public ApiResult modifyBookInfo(Book book) {
        ...
        try{
            ...
            String upSql = "UPDATE  book SET category = ?, title = ?, press = ?, publish_year = ?, author = ?, price = ? WHERE book_id = ?";
            PreparedStatement upStatement = connector.getConn().prepareStatement(upSql);
            upStatement.setString(1,book.getCategory());
            upStatement.setString(2,book.getTitle());
            upStatement.setString(3,book.getPress());
            upStatement.setInt(4,book.getPublishYear());
            upStatement.setString(5,book.getAuthor());
            upStatement.setDouble(6,book.getPrice());
            upStatement.setInt(7,book.getBookId());
    ...
    @Override//done
    public ApiResult borrowBook(Borrow borrow) {//这个写完了还没有测试，似乎要先写register card
        try {//应该使用borrow里面的time！不要修改borrow的信息，不要设置return time
            ...
            if (selResult.next()) {//有没还的书，返回false
                ...
                if (sResult.next()) {
                    ...
                    if (row == 1) {
                        connector.getConn().commit();
                        return new ApiResult(true, "success to borrow book");
                    } else {
                        connector.getConn().rollback();
...
//            String selSql = "SELECT * FROM borrow WHERE card_id = ? AND book_id = ?";
            ...
            if (selResult.next()) {//如果此书有未还的
                ...
                if (rows == 1) {
                    ...
                    if (row == 1) {
                        connector.getConn().commit();
                        return new ApiResult(true, "success to return book");
                    } else {
                        connector.getConn().rollback();
