...
public class LibraryTest {
        ...
        public void bookRegisterTest() {
            ...
            for (int i = 0; i < originBookList.size(); i++) {
                if (actualBookList.contains(originBookList.get(i))) {
                    ...
                    actualBookList.add(originBookList.get(i));//把originBookList的第i个加进去
            ...
            /* generate some books */
            for (int i = 0; i < 50; i++) {
                Book b = RandomData.randomBook();//放一些随机的书
            ...
            for (int i = 0; i < 10; i++) {
                int dupIndex = RandomUtils.nextInt(0, originBookList.size());//获得一个随机的下表
                Book ob = originBookList.get(dupIndex);//获得一个随机的元素，是重复的
                if (bookValid.get(i)) {
                    Assert.assertTrue(actualBookList.contains(ob));
                    Book cb = ob.clone();
                    // randomly change some attributes
                    if (RandomUtils.nextBoolean()) {
                        cb.setStock(RandomData.randomStock());//随便设置一下数量和价格
                        cb.setPrice(RandomData.randomPrice());
                    }
                    Assert.assertTrue(actualBookList.contains(cb));
                    originBookList.add(cb);//把重复的例子放到origin book
                    bookValid.add(false);
                }
            }
            Assert.assertEquals(originBookList.size(), bookValid.size());
            ...
            for (int i = 0; i < originBookList.size(); i++) {
                ...
                if (bookValid.get(i)) {
                    ...
                    Assert.assertFalse(library.storeBook(book).ok);//把书存进去
                }
            }
    ...
    public void borrowAndReturnBookTest() {
        ...
        /* return this book */
        // corner case, for return_time > borrow_time 提供了错误的借书信息
        Borrow nt = new Borrow(b0, c0);
        nt.setReturnTime(666);//设置一个很早的还书时间
