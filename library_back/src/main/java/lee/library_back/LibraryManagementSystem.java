...
public interface LibraryManagementSystem {
     ...
     */
    ApiResult storeBook(Book book);//存一本新书 done
     ...
     */
    ApiResult incBookStock(int bookId, int deltaStock);//done
     ...
     */
    ApiResult storeBook(List<Book> books);//done
     ...
     * the book or has returned it. 只要指定的书归还了就可以继续借书
     *
     ...
     *
     * @return query results should be returned by ApiResult.payload
     *         and should be an instance of {link queriesCardList}
     */
    ApiResult showCards();
